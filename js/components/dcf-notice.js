...
export default class DCFNotice {
  ...
  noticeContainerClassList = [ 'dcf-d-grid', 'dcf-ai-start', 'dcf-w-max-xl', 'dcf-ml-auto', 'dcf-mr-auto', 'dcf-mb-6', 'dcf-rounded' ];
  closeNoticeBtnClassList = [ 'dcf-btn', 'dcf-btn-inverse-tertiary', 'dcf-lh-1' ];
  ...
  constructor(notice, options = {}) {
    ...
    if (this.notice.id === '') {
      this.notice.id = this.uuid.concat('-notice');
    }
    ...
    const all_headings = notice.getElementsByTagName('h2');
    this.heading = all_headings[0] || document.createElement('h2');
    this.heading.classList.add('dcf-notice-heading', 'dcf-txt-h6', 'dcf-mb-0');
    if (this.heading.id === '') {
      this.heading.id = this.uuid.concat('-notice-heading');
    }
    ...
    // set notice message
    const all_messages = notice.getElementsByTagName('div');
    this.message = document.createElement('div');
    if (all_messages[0]) {
      this.message = all_messages[0].cloneNode(true);
    }
